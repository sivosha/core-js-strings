...
function formatTime(minutes, seconds) {
  if (
    ...
    throw new Error(
      'Invalid input: minutes and seconds must be non-negative integers, and seconds must be less than 60.'
    );
  }
...
function encodeToRot13(str) {
  return str.replace(/[A-Za-z]/g, (char) => {
    const base = char <= 'Z' ? 'A'.charCodeAt(0) : 'a'.charCodeAt(0); // Determine case
    return String.fromCharCode(((char.charCodeAt(0) - base + 13) % 26) + base); // Shift by 13
  });
}
